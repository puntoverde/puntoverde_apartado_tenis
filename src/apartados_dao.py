...
class ApartadoDao:
                ...
                # //Para Gym 
                # //+ "WHERE espacio_deportivo.estatus = 1 AND espacio_deportivo.terminal IN (2) AND WEEKDAY(NOW()) = espacio_deportivo_horario.dia_semana  AND now() >= espacio_deportivo_horario.hora1 AND now() <= espacio_deportivo_horario.hora2");
                # 
                # //Para Recepción
    ...
    def get_espacio_deportivo():
        ...
        cursor.execute("""
                       SELECT
                            ...
                            IFNULL(espacio_deportivo.limite_edad,0) AS limite_edad
    ...
    def get_equipos_by_espacio_deportivo(id_espacio_deportivo):
        ...
        cursor.execute("""                        
                       SELECT 
                            ...
                            imagenes.ruta_app, 
                            equipo.estatus,
                            max(apartados.fecha_fin) as disponible
                        FROM equipo 
                        INNER JOIN imagenes ON equipo.cve_imagen = imagenes.cve_imagen
                        left join apartados on equipo.cve_equipo= apartados.cve_equipo AND fecha_fin > now() AND apartados.estatus = 1
                        WHERE equipo.estatus = 1 AND equipo.cve_espacio_deportivo=%s 
                        group by equipo.cve_equipo
    ...
    def get_hora_disponible(id_equipo):
        ...
        if(len(data)==0):
            ...
            return hora_final.format('hh:mm')
        elif(len(data)==2):
    ...
    @staticmethod
    def get_huella(id_socio):
        con=Conexion()
        con.get_conexion()
        cursor=con.conexion.cursor()
        # select socios.foto_socio from socios where socios.cve_socio =11499 and socios.estatus=1 limit 1
        cursor.execute("SELECT huella.huella FROM huella WHERE huella.cveSocio=%s LIMIT 1",(id_socio,))
        data=cursor.fetchone()
        con.close_conexion()
        return data
    ...
    def registrar_apartado(id_usuario,id_equipo,tiempo,inicio,fin):
        ...
        cursor=con.conexion.cursor()            
        cursor.execute("""
                        INSERT INTO apartados(cve_equipo,cve_socio,fecha_registro,fecha_inicio,fecha_fin,estatus,mac) 
                        VALUES (%s, %s, NOW(), %s ,%s,1,%s)
                       """,(id_equipo,id_usuario,inicio,fin,mac,))
        # cursor.execute("""
    ...
    def registrar_apartado_dos(id_usuarios,id_equipo,tiempo,inicio,fin):
        ...
        cursor.execute("""
                        INSERT INTO apartados(cve_equipo,cve_socio,fecha_registro,fecha_inicio,fecha_fin,estatus,mac) 
                        VALUES (%s, %s, NOW(), %s ,%s,1,%s)
                       """,(id_equipo,id_usuarios[0],inicio,fin,mac,))
        id_apartado=cursor.lastrowid
